...
VM_NAME="pfSense${node_number}"
ISO_PATH="/var/lib/vz/template/iso/$iso_name"
MEMORY=1536
CPU="host"
CORES=2
SCSIHW="virtio-scsi-pci"
NET_CONFIG="virtio,bridge=vmbr0,macaddr=00:24:18:0A:${node_number}B:DE"
...
# Destroy
echo -e "${C}Stopping and destroying firewall $VM_ID (if it exists)${NC}"
sudo qm stop $VM_ID || true
sudo qm destroy $VM_ID || true

# Create
echo -e "${C}Creating firewall $VM_ID${NC}"
sudo qm create $VM_ID --name "$VM_NAME" --ostype other --memory $MEMORY --cpu $CPU --cores $CORES --scsihw $SCSIHW --virtio0 "$STORAGE:8,discard=on" --net0 "$NET_CONFIG"
...
echo -e "${G}VM ${L}$VM_ID ${G}(${L}$VM_NAME${G}) has been created successfully!${NC}
${C}After manual installation, poweroff the machine and execute pfSense Postinstall.${NC}"
