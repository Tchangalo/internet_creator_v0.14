...
def general_setup():
    if request.method == 'POST':
        ...
        if 'create_pfsense' in request.form:
            ...
            if not pfs_postinstall_node_no:
                errors.append("The field 'PVE Node No' was left empty!")
            ...
            if errors:
                for error in errors:
                    flash(error, "error")
                return redirect(url_for('general_setup'))
            call_script(GENERAL_SETUP_DIR, 'pfs_postinstall.sh', "postinstall_pfsense", pfs_postinstall_node_no)
